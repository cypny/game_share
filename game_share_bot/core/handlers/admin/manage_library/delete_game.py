...
from game_share_bot.core.keyboards import confirmation_kb, return_to_admin_main_panel_kb
...
async def request_game_id(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "Это экспериментальная функция. Наверное, она может что-то сломать\n\n"
...
async def handle_id_and_request_confirmation(message: Message, session: AsyncSession, state: FSMContext):
    game = await try_get_game_by_id_in_message(message, session, return_to_admin_main_panel_kb())

    have_disc = await DiscRepository(session).get_by_field("game_id", game.id)
    if have_disc:
        return await message.answer(
            "Найдены диски, привязанные к игре\nНевозможно удалить игру с привязанными дисками",
...
@router.callback_query(DeleteGameState.waiting_for_confirmation, ConfirmationCallback.filter_confirmed(), IsAdmin())
async def delete_game(callback: CallbackQuery, session: AsyncSession, state: FSMContext):
    ...
    if result:
        await callback.message.edit_text("Игра успешно удалена", reply_markup=return_to_admin_manage_library_panel_kb())
    else:
        await callback.message.edit_text("Что-то пошло не по плану(",
                                         reply_markup=return_to_admin_manage_library_panel_kb())
    await state.clear()


@router.callback_query(DeleteGameState.waiting_for_confirmation, ConfirmationCallback.filter_canceled())
