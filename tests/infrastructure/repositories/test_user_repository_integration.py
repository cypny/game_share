...
class TestUserRepositoryIntegration:
    ...
    async def test_try_create_duplicate_phone(self, test_session):
        ...
        # Второй пользователь с тем же телефоном
    ...
    async def test_try_create_duplicate_tg_id(self, test_session):
        ...
        # Второй пользователь с тем же TG ID
        user2 = await repo.try_create(
            tg_id=123,
            phone="+2222222222",
            name="user2"
    ...
    async def test_make_admin_success(self, test_session):
        ...
        # Проверяем, что пользователь стал админом (используем role вместо is_admin)
    ...
    async def test_get_all_users(self, test_session):
        ...
        assert len(users) == 3  # Теперь только 3 пользователя, так как база очищается
    ...
    async def test_get_all_admins(self, test_session):
        ...
        await repo.try_create(tg_id=1, phone="+1111111111", name="user1")
        admin_user = await repo.try_create(tg_id=2, phone="+2222222222", name="admin_user")

        # Назначаем второго пользователя админом
